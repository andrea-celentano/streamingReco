...
#include <EIC_3x3_cosmics_trigger/EIC3x3CalTrack.h>
...
EIC_3x3Processor::EIC_3x3Processor() {
    SetTypeName(NAME_OF_THIS); // Provide JANA with this class's name
}

void EIC_3x3Processor::Init() {

    // Open TFiles, set up TTree branches, etc

    LOG << "EIC_3x3Processor::Init" << LOG_END;
    ...
    std::string outFileName = fileAlreadyOpen ? "":"EIC_3x3.root";
    japp->SetDefaultParameter("EIC_3x3:output_file", outFileName, "root file name for EIC_3x3 plugin. (n.b. if not set and file open by another plugin, this will just create a EIC_3x3 directory there to write to)");

    // Optionally open output file
	if (! outFileName.empty() ) {
		std::cout << "JEventProcessor_EIC_3x3: output file name: " << this << " " << outFileName << std::endl;
		fout = new TFile(outFileName.c_str(), "recreate");
    }else{
        // If not opening our own output root file then create a
        // directory for all of our root objects
        gDirectory->mkdir("EIC_3x3")->cd();
    }
    ...
    tcaltrack = new TTree("caltrack","EIC3x3Caltrack");
...
void EIC_3x3Processor::Process(const std::shared_ptr<const JEvent> &event) {
    // LOG << "EIC_3x3Processor::Process, Event #" << event->GetEventNumber() << LOG_END;
    
    auto fa250hits       = event->Get<fa250VTPMode7Hit>();
    auto calhits         = event->Get<HallDCalHit>();
    auto caltracks       = event->Get<EIC3x3CalTrack>();
    ...
    try{ cosmic_trigger  = event->GetSingle<TriggerDecision>("EIC3x3Cal_cosmics"); } catch(...) {}  // optional
...
void EIC_3x3Processor::Finish() {
    // Close any resources
    LOG << "EIC_3x3Processor::Finish" << LOG_END;
