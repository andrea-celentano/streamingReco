#include "EIC_5x5CalCluster_factory_hdbscan.h"
...
bool EIC_5x5CalCluster_factory_hdbscan::compareHits(const HallDCalHit* a, const HallDCalHit* b) {
	return (a->getHitEnergy() > b->getHitEnergy());
}

EIC_5x5CalCluster_factory_hdbscan::EIC_5x5CalCluster_factory_hdbscan() {
...
void EIC_5x5CalCluster_factory_hdbscan::CorrectClusterEnergy(EIC_5x5CalCluster *clus) {
	const HallDCalHit *seed = clus->getHit(0);

	// Disable energy correction for now. DL 3/17/2023
...
void EIC_5x5CalCluster_factory_hdbscan::Init() {

}

void EIC_5x5CalCluster_factory_hdbscan::ChangeRun(const std::shared_ptr<const JEvent> &aEvent) {

	// The following was copied from the original FT source, but is commented out since we
	// are not currently set up to use it.
...
void EIC_5x5CalCluster_factory_hdbscan::Process(const std::shared_ptr<const JEvent> &aEvent) {

	//Loop su gli hits di un evento
	std::vector<const HallDCalHit*> hits = aEvent->Get<const HallDCalHit>();

	std::sort(hits.begin(), hits.end(), EIC_5x5CalCluster_factory_hdbscan::compareHits);
	...
	//-------------- add real hits ----------------//
	for(auto hit : hits){
		tmp_point.x = hit->getHitIX();
		tmp_point.y = hit->getHitIY();
	...
	//cout<<"::::: hdbscan with EIC_5x5CalCluster_factory"<<endl;
	if(prepr_points.size()>0) res_clus_hdbscan = hdbscan_main(prepr_points, dim, par1, par2);

	std::vector<EIC_5x5CalCluster*> clusters;
	...
	for(int i=0; i <res_clus_hdbscan.size(); i++){
		...
		EIC_5x5CalCluster *cluster = new EIC_5x5CalCluster();//clusters.size()
